...
public class productPage extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            final int limit = 4;
            int offset = (page-1)*4;
            int totalProduct = dao.currentProduct();
            int currentProduct = dao.currentProduct();
            int endOfPage = (int) Math.ceil((double) totalProduct/limit);
            /*-----------------------------*/
            list=dao.pagePaging(offset, limit);
            listCate=dao.listCategory();
            /*-----------------------------*/
            request.setAttribute("productList", list);
            request.setAttribute("listCate", listCate);
            request.setAttribute("currentProduct", currentProduct);
            request.setAttribute("endOfPage", endOfPage);
